...
%.3f_%s, with a completeness of %s in the highest resolution shell
%.3f_%s, with a completeness of %s in the highest resolution shell
%.3f_%s, with a completeness of %s in the highest resolution shell
...
''' %(r1[0],r1[1], r1[2], r2[0],r2[1], r2[2], r3[0],r3[1], r3[2])  
    ...
    def getMaxResolution(self, thr, cl, ano):
        highResLine = []
        with open(self.Workdir+'/cc_Cluster_%.3f_%s_%s/XSCALE.LP'%(float(thr), cl, ano), 'r') as LogFile:
            for line in LogFile:
