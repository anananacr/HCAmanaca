...
def hcarun(inp,out,points,dist):
	...
	'''
	#Not used code communication with user set parameters directly in main function
	...
	f.close()
	'''
	#Not used it was usede to fin clusters merged in the the directory
	...
	'''
	#Plots for inner and outer shell, substituted for resolution shells plots in meritplot
	print("Plot Completeness")
	HCAplot.plot('completeness', 'fast', 0)

	print("Plot CC1/2")
	HCAplot.plot('cc', 'fast', 0)

	print("Plot Rmeas")
	HCAplot.plot('r', 'fast', 0)

	print("Plot CC1/2 vs Compl (Inner)")
	HCAplot.map("fast",dist, "Inner",0)
	print("Plot CC1/2 vs Compl (Outer)")
	HCAplot.map("fast",dist, "Outer",0)

	print("Dynamic plot")
	HCAplot.dyn_plot('fast', dist, 'Inner',0)
	HCAplot.dyn_plot('fast', dist, 'Outer',0)
	...
	sub.call("mv *.png "+meudiretorio, shell=True)
	'''
	Summary function of ccCluster tells 3 best clusters.
	'''
	#summary.main()
	...
	"""
	Shuffle function of ccCluster random tests of XSCALE file reference for scaling.
	"""
	...
	"""
	Move results to an upper folder ../test_cccluster/results_script you should have this path or change it here.
	"""
...
def main():
	...
	"""
	Path where directories of each subdataset XDS_ASCII.HKL file is located.
	"""
	#inp='../test_cccluster/data/20200227_lys_hr_n2_w5'
	inp='../ame_n2_w5'
	
	out='20210815_ame_test/'
	number=5
	dist='cc'
